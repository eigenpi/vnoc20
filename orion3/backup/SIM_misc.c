...
static int SIM_bus_bitwidth(int encoding, u_int data_width, u_int grp_width)
{
	if (encoding && encoding < BUS_MAX_ENC)
		switch (encoding) {
			...
			default: printf ("error bus encoding bitwidth\n");	/* some error handler */
		}
	else
		return -1;
}
...
LIB_Type_max_uint SIM_bus_state(SIM_bus_t *bus, LIB_Type_max_uint old_data, LIB_Type_max_uint old_state, LIB_Type_max_uint new_data)
{
	...
	switch (bus->encoding) {
		...
		default: printf ("error bus encoding state\n");	/* some error handler */
	}
}
 ...
 */
int SIM_bus_init(SIM_bus_t *bus, int model, int encoding, 
    u_int width, u_int grp_width, u_int n_snd, u_int n_rcv, 
        ...
        switch (model) {
            ...
            case GENERIC_BUS:
                if ((bus->encoding = encoding) && encoding < BUS_MAX_ENC) {
                    bus->e_switch = EnergyFactor * (SIM_generic_bus_cap(n_snd, n_rcv, length, time) / 2);
                    /* sanity check */
...
{
    //printf(" bus n_switch: %d bus e_switch: %g \n", bus->n_switch, bus->e_switch);
...
int SIM_fpfp_init(SIM_ff_t *ff, int model, double load)
{
	...
	if ((ff->model = model) && model < FF_MAX_MODEL) {
		switch (model) {
			...
			default: printf ("error fpfp init \n");	/* some error handler */
		}
