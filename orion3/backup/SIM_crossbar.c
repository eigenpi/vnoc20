...
int SIM_crossbar_init(SIM_crossbar_t *crsbar, int model, 
    u_int n_in, u_int n_out, u_int in_seg, u_int out_seg, 
    u_int data_width, u_int degree, int connect_type, 
    int trans_type, double in_len, double out_len, double *req_len)
{
    ...
    if ((crsbar->model = model) && model < CROSSBAR_MAX_MODEL) {
        ...
        switch (model) {
            ...
            default: printf ("error crossbar_init\n");  /* some error handler */
        }
...
int SIM_crossbar_record(SIM_crossbar_t *xb, int io, 
    LIB_Type_max_uint new_data, LIB_Type_max_uint old_data, 
    u_int new_port, u_int old_port)
{
    switch (xb->model) {
        case MATRIX_CROSSBAR:
            if (io) /* input port */
        ...
        default: printf ("error crossbar_record\n");    /* some error handler */
    }
...
double SIM_crossbar_stat_energy(SIM_crossbar_t *crsbar, int print_depth, char *path, int max_avg, double n_data)
{
    ...
    if (n_data > crsbar->n_out) {
        //printf(" n_data = %d crsbar n_out = %d", n_data, crsbar->n_out);
	...
	switch (crsbar->model) {
		...
		default: printf ("error crossbar_stat_energy\n");	/* some error handler */
	}
