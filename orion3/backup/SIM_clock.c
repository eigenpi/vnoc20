...
double SIM_total_during_n_cycles_clockEnergy(SIM_router_info_t *info, SIM_router_power_t *router)
{
    // Cris: this function computes really the total dynamic energy
    // consumed by the clock of this router during "that" many (i.e., n)
    // simulation cycles;
    // I need this in order to accumulate energy consumed by clock
    // over a simulation length of n cycles; each simulation cycle
    // the router consumes an amount of energy given by SIM_total_clockEnergy();
    // this is also useful in computing energy consumed by clock during a
    // period when I scale down the frequency during DVFS;
    // I divide by 4 to account for the four router pipe stages; 

    double energy_during_n_cycles = 0.0;
    energy_during_n_cycles = 
        (info->n_simulation_cycles / 4) * SIM_total_clockEnergy(info, router);
    return energy_during_n_cycles;
}
void SIM_simulation_cycles_record(SIM_router_info_t *info)
{
    info->n_simulation_cycles = info->n_simulation_cycles + 1;
}
