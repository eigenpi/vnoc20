...
int SIM_router_power_init(SIM_router_info_t *info, SIM_router_power_t *router)
{
    ...
    if (info->out_buf) {
        ...
        //printf("called SIM_array_power_init for out_buf\n");
    }
    ...
    if (info->central_buf) {
        ...
        //printf("called SIM_array_power_init for central_buf\n");
    ...
    if(info->vc_allocator_type == ONE_STAGE_ARB && info->n_v_channel > 1 && info->n_in > 1){
        ...
        //printf("called SIM_array_power_init for vc_allocator\n");
        /* static power */
 ...
 */
double SIM_router_stat_energy(SIM_router_info_t *info, SIM_router_power_t *router, 
    int print_depth, char *path, int max_avg, double e_fin, int plot_flag, double freq)
{
    double Eavg = 0.0, Eatomic, Estruct, Estatic = 0;
    ...
    if (info->in_buf) {
        ...
        //printf(" e_in_buf_rw: %d \n", e_in_buf_rw);
        //printf(" in_buf energy: %g \n", Eavg-energy_prev); energy_prev = Eavg;
    ...
    if (info->central_buf) {
        ...
        SIM_res_path(path, path_len);
        Eavg += Estruct;
        //printf(" central_buf energy: %g \n", Eavg-energy_prev); energy_prev = Eavg;
    }

    //printf("\n Eavg: %g freq: %g", Eavg, freq);
    Pbuf_dyn = Eavg * freq;
    Pbuf_static = router->I_buf_static * Vdd * SCALE_S;
    Pbuf = Pbuf_dyn + Pbuf_static;
    //printf(" Pbuf_dyn: %g \n", Pbuf_dyn);
    //printf(" Pbuf_static: %g \n", Pbuf_static);
    ...
    if (PARM(TECH_POINT) <= 90) {
        //energy_prev = Eavg; // cris
        Eavg += SIM_total_clockEnergy(info, router);
        //printf("\n Eavg: %g", Eavg - energy_prev); // cris
        ...
        //printf("\n Pclock_dyn: %g Pclock_static: %g", Pclock_dyn, Pclock_static);
    }
